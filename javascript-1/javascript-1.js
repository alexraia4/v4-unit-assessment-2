...
//CODE HERE
const compareNums = (herp, derp) => Math.max(herp, derp);
...
//CODE HERE
const bestMovie = (derp) => `${derp} is the best movie ever!`;
...
for (let i = 0; i < classes.length; i++){
    for (let k in classes[i]){
        if (k === 'inPerson' && classes[i][k] === true){
            classes[i][k] = false;
        }
        if (k === 'homework' && classes[i][k] === true){
            classes[i][k] = false;
        }
    }
}
...
//CODE HERE
let phone1 = new Phone('Nokia', 'erTyXx324', 2548, 'Aquamarine', true);
  
let phone2 = new Phone('Samsung', '4tYy57Uo', 1592, 'Cinnabar', true);
  
let phone3 = new Phone('Herp', 'Derp', 0, 'Slurp', false);
